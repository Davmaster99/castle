...
//Checking on Michelin's restaurants in france 
var pagesToScrap = [];
    ...
    request({
      ...
      $('.ds-1col.node.node--poi.view-mode-poi_card.node--poi-card').each(function(){
        var currentRestaurant = {
          ...
          urls:  'https://restaurant.michelin.fr' + $('a', this).attr('href')
        };
...
exports.get = function() {
  ...
  .then(function(results){
    jsonfile.writeFile('output/Restaurants.json', results, {spaces: 2}, function(err){
      if(err){
        console.error(err);
      }
...
function getAddress(restaurants, idx) {
  return new Promise(function(resolve, reject){
    console.log('[' + (idx+1) + '/' + restaurants.length + '] Get on  ' + restaurants[idx].urls.michelin);
    request({
      method: 'GET',
...
exports.getAllAddresses = function(restaurants) {
  ...
  .then(function(restaurants){
    jsonfile.writeFile('output/Restaurants_list.json', restaurants, {spaces: 2}, function(err){
      if(err){
        console.error(err);
      }
